...
Open [KaraokeKonverter on AWS](https://d2j5ofy3dzbrbx.cloudfront.net). Use the application access code that I share privately. No AWS account, local installation, Google API key or music-provider sign-in is needed to use this hosted demonstration.
...
If a source is blocked or a provider rejects a request, send me the displayed error and approximate time. Do not include the access code in screenshots or GitHub issues. One small conversion per source is sufficient for a quick demonstration; repeated searches consume my project quota.

## Review the engineering work

Start with the [architecture](architecture.md), then the [monitoring runbook](monitoring.md) and [validation record](validation.md). The [CI workflow](../.github/workflows/ci.yml) shows automatic tests and build checks. I keep AWS console access private; the public [monitoring evidence summary](evidence/monitoring-acceptance.md) records my dashboard and alert observations.

## Sharing my demo

I share the repository and demo URLs with a reviewer and provide the existing app access code privately. The current code is shared across demo visitors; there is no per-user revocation or automatic invitation expiry. Rotate it in Secrets Manager when access should end. The YouTube key and AWS deployment credentials are never part of the reviewer handoff.
