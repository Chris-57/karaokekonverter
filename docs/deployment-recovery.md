...
Use this runbook with [deployment automation](deployment-automation.md). The first live restore is an acceptance exercise, not something established by the offline tests.
...
| Snapshot or change-set validation | The existing app normally remains unchanged; private preparation files/change sets may exist. Inspect the first error. Do not recreate the application stack. |
...
4. Watch the restore run. It loads the saved template and site files, verifies their checksums and account/stack identity, captures the current deployment where possible, then follows the same change-set restrictions and post-deploy checks. It uses current trusted deployment code and does not rebuild the old application commit.
...
The two application roles and the availability role cannot be altered by this workflow. Consequently a restore cannot undo a separately applied IAM or resource-replacement migration; that needs an operator review. Job records and secret values are never part of a release snapshot.
